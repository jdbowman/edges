...
def checkWeather(argv):
  """
  Read the weather sensors if they are connected to this host and print the values
  to stdout.
  """
  ...
  result = True
  bSave = False
  helpString =  '\nusage: checkWeather.py [options]\n\n' \
                '\tRead the weather sensors if they are connected\n' \
  ...
  # Abort if we are on the wrong host to check weather sensors
  if socket.gethostname() != 'edges-pc-high':
    
    print('ERROR: This host is not connected to the weather sensors.  Aborted.')
    return False
  ...
  # Check the sensors
    ...
    if bSave:
      ...
      result = d.writeRecord( 'environ', 'weather', 
