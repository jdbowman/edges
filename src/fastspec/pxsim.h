...
class PXSim : public Digitizer {
  ...
  private:
    ...
    digitizer_callback_fptr     m_pCallback;
    bool                        m_bStop;
  ...
  public:
    ...
    bool acquire(unsigned long uNumSamplesToTransfer) {
      ...
      while ((uNumSamples < uNumSamplesToTransfer) && !m_bStop) {
        ...
        uNumSamples += m_pCallback(m_pBuffer, m_uSamplesPerTransfer, uNumSamples);
    ...
    void setCallback(digitizer_callback_fptr pFunc) { m_pCallback = pFunc; }
