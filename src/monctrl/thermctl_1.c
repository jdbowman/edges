...
//      if(mm==12) {mode = 0x1d; t=0;} // Disable active control
    ...
    //-----------------------------------------------
    status = -1;
    if ((file1 = fopen("/home/loco/edges/data/thermlog_mid.txt", "a")) == NULL) 
    {
      if ((file1 = fopen("/home/loco/edges/data/thermlog_mid.txt", "w")) == NULL) 
      {
        printf("cannot write %s\n", "/home/loco/edges/data/thermlog_mid.txt");
        return 0;
      }
      status = 1;
    } else {
