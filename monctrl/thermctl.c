...
{ 
  ...
  double outset,outbw,set,tmp,pwr,bw,gain,deriv,volt,swvolt,enabled,secs,temp_limit;
  ...
  while (usb >= 0 && usb < 3 && found == 0)
  {
    ...
    //-----------------------------------------------
    outset=outbw=set=tmp=pwr=gain=bw=deriv=volt=swvolt=enabled=0;
    for(mm=0;mm<10;mm++)
    {
      t=0; 
      if(mm==0) {mode= 0x10;t=25*100;}// set temperature point
      if(mm==1) {mode= 0x11;t=60;}    // set bandwidth
      if(mm==2) mode= 0x40;         // read set point
      if(mm==3) mode= 0x01;         // read temp
      if(mm==4) mode= 0x04;         // read pwr
      if(mm==5) mode= 0x41;         // read bw
      if(mm==6) mode= 0x42;         // read gain
      if(mm==7) mode= 0x43;         // read deriv
      if(mm==8) mode= 0x46;         // read voltage
      if(mm==9) mode= 0x02;         // read switch voltage
      if(mm==10) {mode = 0x1d; t=0;} // Disable active control
 //     if(mm==10) {mode = 0x1d; t=1;} // Enable active control
      ...
      if (mm==1) outbw = t/10.0;
      if (mm==2) set = t/100.0;
      if (mm==3) tmp = t/100.0;
      if (mm==4) pwr = t*100.0/683.0;
      if (mm==5) bw = t/10.0;
      if (mm==6) gain = t/10.0;
      if (mm==7) deriv = t/10.0;
      if (mm==8) volt = t/1000.0;
      if (mm==9) swvolt = t/1000.0;
      if (mm==10) enabled = t;
    ...
    printf(" outset: %5.2f\n outbw: %5.2f\n set: %5.2f\n temp: %5.2f\n pwr: %5.2f\n bandwidth: %5.2f\n gain: %5.2f\n deriv: %.2f\n voltage: %.2f\n switch voltage: %.2f\n enabled (always returns zero): %5.2f\n", outset, outbw, set, tmp, pwr, bw, gain, deriv, volt, swvolt, enabled);
  ...
  if(tmp > temp_limit) {
    ...
    system("/media/DATA/EDGES_codes/edges_c/power_switch8 -cmd 'gpset 1 0'");
    system("/media/DATA/EDGES_codes/edges_c/power_switch8 -cmd 'gpset 1 0'");
