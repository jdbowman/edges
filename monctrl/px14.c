...
int pxrun(int mode)
{
	...
	if(mode == -1)
	{
		...
		printf ("PciAcqPX14 v1.0 - Signatec PX14400 PCI Acquisition Recording Utility\n\n");
		...
		SetTriggerSourcePX14(hBrd,PX14TRIGSRC_INT_CH1); 
	...
	if(mode == 0)
	{
		...
		res = GetPciAcquisitionDataFastPX14(hBrd, DMA_XFER_SAMPLES, dma_bufp, PX14_TRUE);
		if (SIG_SUCCESS != res)
		{
...
int px14run(float spec[], int numacq)
{
    ...
    for(i=0;i<blsiz;i++) 
	{
		win0[i]=win1[i]=win2[i]=win3[i]=0.5*(a0-a1*cos(2.0*PI*i/(blsiz-1))+a2*cos(4.0*PI*i/(blsiz-1))-a3*cos(6.0*PI*i/(blsiz-1)))/32768.0;
	    // 0.5 for px14400
	}
...
//       usleep(100000);  // sleep for 0.1 sec
    if(numacq > 0) 
	{
        for (i = 0; i < blsiz2; i++) 
		{
			spec0[i] = spec1[i] = spec2[i] = spec3[i] = 0.0;
		}
        ...
        for(num=0;num<numacq;num++)
		{
			if(num == 0) 
			{
				...
				for (i = 0; i < NBR; i++)
				{
					LastwaveForm[i] = waveFormArray[i];
				}
