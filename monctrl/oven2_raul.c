...
{ int usbdev;
  int status,i,chk,t,j,chkrec,mode,m,turnoff,turnon,volt12;
  double set;
  char command[16],resp[12],buf[256];
 
  mode = -1; set=25.0;  turnoff = turnon = 0; volt12=0;
  for(m=0;m<argc;m++){
  sscanf(argv[m], "%79s", buf);
   if (strstr(buf, "-mode")) {sscanf(argv[m+1], "%x",&mode);}
   if (strstr(buf, "-temp")) {sscanf(argv[m+1], "%lf",&set); mode=0x10;}
   if (strstr(buf, "-turnoff")) {turnoff = 1; mode=0;}
   if (strstr(buf, "-turnon"))  {turnon = 1;  mode=0;}
   if (strstr(buf, "-volt12"))  {volt12 = 1;  mode=0;}
  }
  ...
  if(mode == -1) {
         ...
         printf("type oven -mode codes above to read\n");
         printf("type oven -turnoff to turnoff\n");
         printf("type oven -turnon to turnon\n");
         printf("type oven -temp 25 to set temperature to 25\n");
  return 0;}

  system("stty -F /dev/ttyUSB1 19200 cs8 -cstopb -parity -icanon min 1 time 1");
  usbdev=0; 
  usbdev = open("/dev/ttyUSB1",O_RDWR);
  printf("usbdev %x\n",usbdev);

  t = set*100;
  sprintf(command,"*00%02x%08x",mode,t);
  if(turnoff) sprintf(command,"*00%02x%08x",0x1d,0);
  if(turnon) sprintf(command,"*00%02x%08x",0x1d,1);
  if(volt12) sprintf(command,"*00%02x%08x",0x16,12000); 
